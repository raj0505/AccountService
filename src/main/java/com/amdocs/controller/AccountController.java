...
import com.amdocs.client.UserClient;
...
public class AccountController {
	...
	@KafkaListener(groupId = "abc", topics = "kafka_user", containerFactory = "userKafkaListenerContainerFactory")
	public User getDataFromProducer(User user) {
...
//using spring cloud
	...
	/*
	 * @GetMapping("/user/{userId}") public List<Account> getAccounts(@RequestBody
	 * User user, @PathVariable("userId") long userId){
	 */

	/*
	 * return this.accountService.getAccountsOfUser(user);
	 * 
	 * }
	 */

	@GetMapping("/user") // localhost:9002/account/user
	public ResponseEntity<List<User>> getAccountUser() {
		List<User> users = this.userClient.getAccountUser();
